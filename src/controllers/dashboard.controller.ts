...
FROM
    ...
    protected async getEvaluatorStats(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
        ...
        try {
            let response: any = {};
            const PendingForApproval = await db.query("SELECT count(challenge_response_id) as 'PendingForApproval' FROM challenge_responses where status = 'SUBMITTED' && (verified_status IS NULL OR verified_status = '')", { type: QueryTypes.SELECT });
            const mentorAcceptedCount = await db.query("SELECT count(challenge_response_id) as 'submitted_count' FROM challenge_responses where status = 'SUBMITTED' && verified_status='ACCEPTED'", { type: QueryTypes.SELECT });
            ...
            if (mentorAcceptedCount instanceof Error) {
                throw mentorAcceptedCount
            }
            ...
            mentorAcceptedCount
            response['mentorAcceptedCount'] = Object.values(mentorAcceptedCount[0]).toString();
    ...
    //fecting team count for the mentor, state, & all
    protected async getteamCount(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
        ...
        try {
            ...
            const { mentor_id, state } = newREQQuery
            if (mentor_id) {
                ...
                FROM
                    ...
                    WHERE og.status='ACTIVE' && og.state = '${state}';`, { type: QueryTypes.SELECT });
            }
            else {
                result = await db.query(`SELECT 
                COUNT(t.team_id) AS teams_count
    ...
    //fecting student count for the mentor, state, & all
    ...
    //fecting ideas count for the mentor
    ...
    //fecting mentor accepted ideas count for the mentor
...
WHERE
    ...
    //fecting mentor course percentage
    ...
    //fecting mentor survey stats (pre and post)
    ...
    //fetching team login details for mentor
    ...
    //fetching student course count for state and all
    protected async getstudentCourseCount(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
        ...
        try {
            ...
            let StudentCoursesCompletedCount
            let started
            if (state) {
                StudentCoursesCompletedCount = await db.query(`SELECT 
                ...
                FROM
                    ...
                    (SELECT 
                        DISTINCT user_id
                    FROM
                        user_topic_progress ) AS temp ON st.user_id = temp.user_id WHERE og.status='ACTIVE' && og.state='${state}';`, { type: QueryTypes.SELECT });
            } else {
                StudentCoursesCompletedCount = await db.query(`SELECT 
                    count(st.student_id) as studentCourseCMP
                FROM
    ...
    //fetching ideas count for state and all
    protected async getideasCount(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
        ...
        try {
            ...
            let submittedCount
            if (state) {
                fullCount = await db.query(`SELECT 
                ...
                FROM
                    ...
                    (SELECT 
                        team_id, status
                    FROM
                        challenge_responses) AS temp ON te.team_id = temp.team_id WHERE og.status='ACTIVE' && og.state='${state}'`, { type: QueryTypes.SELECT });
                ...
                FROM
                    ...
                    FROM
                        challenge_responses
                    WHERE
                        status = 'SUBMITTED') AS temp ON te.team_id = temp.team_id WHERE og.status='ACTIVE' && og.state='${state}'`, { type: QueryTypes.SELECT })
            }
            else {
                fullCount = await db.query(`SELECT 
                    count(te.team_id) as initiated
    ...
    //fetching mentor male and female count for state and all
    ...
    //fetching student male and female count for state and all
    ...
    //fetching school count for state and all
    ...
    //fetching mentor course count for state and all
...
WHERE
    ...
    //fetching Atl and nonatl count for state and all
    ...
    //fetching studetn course count for state and all
    ...
    //fetching mentor survey count for state, all
...
WHERE
    ...
    //fetching student survey count for state, all
